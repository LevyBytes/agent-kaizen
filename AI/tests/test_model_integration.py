...
GATED: runs only with KAIZEN_RUN_LIVE=1, because it loads real models (download / VRAM / heat):
    KAIZEN_RUN_LIVE=1 <venv>/python -m unittest discover -s AI/tests -t AI/tests -p test_model_integration.py
...
class LiveModelIntegrationTest(unittest.TestCase):
    ...
    def tearDownClass(cls) -> None:
        _kz("K7")  # delete every is_test=1 record this suite wrote (document cascades to its chunks)
        shutil.rmtree(getattr(cls, "_tmp", None) or ".", ignore_errors=True)
