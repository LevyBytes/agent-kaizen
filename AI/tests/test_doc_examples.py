...
_FENCE_RE = re.compile(r"^[ \t]*```(?:powershell|sh)\s*$(.*?)^[ \t]*```\s*$", re.MULTILINE | re.DOTALL)
...
def _doc_commands(text: str) -> tuple[list[list[str]], int]:
    ...
    for block in _FENCE_RE.findall(text):
        for line in block.splitlines():
            ...
            if not starts:
                continue
            commands.append(tokens[starts[0] + 1 :])
...
class DocExamplesTest(unittest.TestCase):
    ...
    def _run_document(self, rel_path: str) -> None:
        ...
        commands, skipped = _doc_commands(text)
