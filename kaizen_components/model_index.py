...
def list_models() -> list[dict[str, Any]]:
    ...
    rows = fetch_all(
        "SELECT embedding_model, COUNT(*), MAX(dim), SUM(dim), MAX(created_at) "
    ...
    return [
        ...
        for model, count, dim, sum_dim, latest in rows
    ]
...
def activate_model(model: str, *, keep_all: bool = False) -> dict[str, Any]:
    ...
    retrieval."""
    total_row = fetch_one("SELECT COUNT(*) FROM evidence_chunks")
    total = int(total_row[0]) if total_row else 0
    indexed_row = fetch_one("SELECT COUNT(*) FROM chunk_embeddings WHERE embedding_model = ?", (model,))
...
def _prune(args: Any) -> dict[str, Any]:
    ...
    targets = [t for t in targets if t and t != active]  # the active index is never prunable
