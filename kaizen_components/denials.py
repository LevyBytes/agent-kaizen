"""Define structured CLI denials and their copy-paste remediation examples."""
...
class KaizenDenied(Exception):
    """Carry a denial code, JSON fields, and exit code while preserving Exception.args."""
    ...
    def payload(self) -> dict[str, Any]:
        """Return the JSON-emittable ``DENIED`` payload."""
        return {"status": "DENIED", "code": self.code, **self.fields}


def usage_denial(message: str, **fields: Any) -> KaizenDenied:
    """Build an exit-2 usage denial with an overridable resubmission remedy."""
    return KaizenDenied(
        "DENIED_USAGE",
...
def remedy_example(code: str, operation: str | None) -> str | None:
    """Return the remedy for ``code`` with its operation placeholder resolved, if known."""
    template = REMEDY_EXAMPLES.get(code)
    return template.replace("{op}", operation or "<operation>") if template else None
