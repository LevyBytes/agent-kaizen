...
def kz(*args: str) -> tuple[int, dict]:
    """Run a kaizen op --json and return (rc, payload). Robust to MCP server log noise: prefers the
    clean stdout JSON (OK path); on the DENIED/ERROR path the JSON is on stderr amid server logs, so
    slice it out by kaizen's pretty-print header."""
    proc = subprocess.run(
    ...
    if start >= 0:
        try:
            return proc.returncode, json.loads(err[start:err.rfind("}") + 1])
        except json.JSONDecodeError:
            pass
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--workflow", default="AI/work/wf-sd15.json", help="API-format workflow with a {{PROMPT}} placeholder")
    ...
    step("Y6 doctor", d.get("status") == "OK" and d.get("cuda") is True,
         f"cuda={d.get('cuda')} gpu={d.get('gpu')} checkpoints={d.get('checkpoints')}")
    ...
    if d.get("code") in ("DENIED_ASSET_MISSING", "DENIED_WORKFLOW_NODES_UNKNOWN"):
        step("Y8 --route api", False,
             f"approval gate fired ({d.get('code')}): place the model and re-run -- {d.get('missing_assets') or d.get('unknown_class_types')}")
    else:
        step("Y8 --route api", d.get("status") == "OK" and d.get("run_status") == "completed",
             f"run_status={d.get('run_status')} id={api_id} outputs={d.get('outputs')}")
    if api_id:
        rc, d = kz("Y2", "--id", api_id)
        r = d.get("record", {})
        ri = r.get("route_info") or {}
        step("Y2 inspect", r.get("status") == "completed" and ri.get("route") == "api",
             f"route={ri.get('route')} runtime_profile={ri.get('runtime_profile')}")

    if a.mcp:
        # 4. bakeoff -> pin winner
        rc, d = kz("Y7", "--action", "bakeoff", "--test")
        step("Y7 bakeoff", d.get("status") == "OK" and bool(d.get("winner")),
             f"winner={d.get('winner')} candidates={d.get('candidates')}")
        # 5. mcp generation
        rc, d = kz("Y8", *gen, "--route", "mcp")
        step("Y8 --route mcp", d.get("status") == "OK" and d.get("run_status") == "completed",
             f"run_status={d.get('run_status')} route={d.get('route')} outputs={d.get('outputs')}")
        # 6. A/B parity
        rc, d = kz("Y9", *gen)
        p = d.get("parity", {})
        step("Y9 A/B parity", d.get("status") == "OK" and p.get("both_completed"),
             f"identical_hashes={p.get('identical_hashes')} latency={p.get('latency_ms')} report={d.get('report')}")

    # cleanup
    if not a.keep:
        rc, d = kz("K7")
        print(f"[cleanup] K7 purge-test removed test records: {d.get('deleted') or d.get('purged') or d.get('counts') or d}")
