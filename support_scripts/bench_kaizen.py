...
Usage:
    python support_scripts/bench_kaizen.py                 # full run, updates docs/ + README
    python support_scripts/bench_kaizen.py --quick         # tiny run, writes to a temp dir
    python support_scripts/bench_kaizen.py --out DIR       # redirect all artifacts (dry run)
    python support_scripts/bench_kaizen.py --from-json P   # re-render artifacts, no measuring
    python support_scripts/bench_kaizen.py --allow-semantic  # keep embed env vars; time E4 --semantic
"""
...
def call_op(*argv: str) -> tuple[int, dict]:
    ...
    raw = out.getvalue().strip() or err.getvalue().strip() or "{}"
    try:
...
def phase_retrieval(bench_root: Path, docs: int, paragraphs: int, repeats: int,
    ...
    if rc == 0 and payload.get("mode") == "semantic" and allow_semantic:
        sem_samples = []
        for i in range(docs):
            for _ in range(repeats):
...
def phase_context_recovery(digest_entries: list[dict]) -> dict:
    ...
    slug = re.sub(r"[^a-z0-9]", "-", str(REPO_ROOT).lower())
    ...
    median_size, median_path = raw_sizes[len(raw_sizes) // 2]
    chars = 0
...
def mermaid_xychart(title: str, x_labels: list[str], series: list[tuple[str, list[float]]],
                    y_label: str = "ms") -> str:
    y_max = max(value for _kind, values in series for value in values)
...
def render_svg_charts(results: dict) -> dict[str, str]:
    """SVG artifacts embedded by the README region; filename -> content."""
    ops = list(results["write_ops"])
    charts = {
...
def render_benchmarks_md(results: dict) -> str:
    ...
    parts = [
        "<!-- Generated by support_scripts/bench_kaizen.py; do not edit by hand. -->",
        "",
        "# Benchmarks",
        "",
        ...
        if transcripts.get("found"):
            ...
            parts.append(fmt_table(
                ...
                [[f"`R0` digest at {ctx['r0_scale']:,} seeded records",
                 ...
                 [f"Median session transcript, text content ({transcripts['count']} sessions)",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.out:
        ...
        out_dir = Path(tempfile.mkdtemp(prefix="kaizen-bench-out-"))
    else:
    ...
    if args.from_json:
        ...
        bench_root = Path(tempfile.mkdtemp(prefix="kaizen-bench-"))
