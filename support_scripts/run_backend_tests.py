...
Usage:
    support_scripts\\run-backend-tests.cmd
    python support_scripts/run_backend_tests.py
    python support_scripts/run_backend_tests.py --no-live    # skip real-weight loaders (fast, no GPU/heat)
    ...
    python support_scripts/run_backend_tests.py --hold-seconds 40
...
def main(argv=None) -> int:
    ...
    ap.add_argument("-v", "--verbose", action="count", default=2)
    args = ap.parse_args(argv)

    if not args.no_live:
        os.environ.setdefault("KAIZEN_RUN_LIVE", "1")   # must be set BEFORE the modules import (they read it)
        os.environ.setdefault("KAIZEN_TORCH_DEVICE", "auto")  # GPU-first for the torch lanes
        preflight()
        if not args.no_gui:
