...
Every test that exercises the database runs `kaizen.py` as a subprocess with `KAIZEN_REPO_ROOT` pointed at a fresh temporary directory. `paths.py` anchors the whole data plane (`kaizen.db`, exports, manifests) on that root, so the tests read and write only throwaway temporary databases and never the project's real `AI/db`. Temporary roots are deleted on teardown.

Operator-driven live utilities such as `bench_kaizen.py` and `comfy_live_verify.py` are not unittest-isolated; follow their explicit scratch/test-record and cleanup contracts before running them.

Tests invoke `sys.executable`, so run the canonical wrapper with the shared Kaizen venv (the one that has `pyturso` installed). The wrapper pins `TEMP`, `TMP`, `TMPDIR`, and Python bytecode under repo-local `AI/work` before the child interpreter imports the suite.
...
- `test_bench_smoke.py` — `tests/bench_kaizen.py --quick` runs green into a temp
