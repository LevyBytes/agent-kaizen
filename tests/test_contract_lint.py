...
class LintTextUnitTest(unittest.TestCase):
    ...
    def test_abbreviations_do_not_create_phantom_sentences(self):
        result = lint_text("Use e.g. Alpha safely. Explain i.e. Beta precisely.")
        self.assertEqual(result["sentence_count"], 2, result)

    def test_library_thresholds_remain_tunable(self):
        default = lint_text("Really ship bounded evidence.")
        tuned = lint_text("Really ship bounded evidence.", hedge_density_max=1.0)
        self.assertEqual(default["verdict"], "fail", default)
        self.assertEqual(tuned["verdict"], "pass", tuned)
