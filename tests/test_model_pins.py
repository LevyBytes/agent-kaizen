...
class ModelPinTest(unittest.TestCase):
    ...
    def test_ollama_installer_and_docs_default_to_qwen_judge(self):
        expected_assignments = {
            "setup/install-ollama.ps1": f"[string]$ChatModel = '{OLLAMA_JUDGE_GGUF}'",
            "setup/install-ollama.sh": f'CHAT_MODEL="{OLLAMA_JUDGE_GGUF}"',
            "setup/OLLAMA.md": f"KAIZEN_LLM_MODEL={OLLAMA_JUDGE_GGUF}",
        }
        for rel, assignment in expected_assignments.items():
            text = (REPO_ROOT / rel).read_text(encoding="utf-8")
            self.assertIn(assignment, text, f"{rel} must assign the judge/chat default to Qwen3.5-9B GGUF")
