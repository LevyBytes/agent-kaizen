...
class MigrationOpsTest(IsolatedDBTest):
    ...
    def test_m3_apply_creates_stub_files_without_empty_backup_manifest(self):
        rc, p = self.kz("M3")
        self.assertEqual(rc, 0, p)
        self.assertEqual(p["status"], "OK")
        self.assertEqual(p["count"], 3, p)
        ...
        self.assertIsNone(p["manifest"], p)
        backups = self.root / "AI" / "work" / "kaizen-v4-migration" / "backups"
        self.assertFalse(backups.exists(), "no pre-existing files means no backup artifact")
