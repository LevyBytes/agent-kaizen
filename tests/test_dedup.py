...
def _seed_evidence_vectors(db_path: str, vectors: list[list[float]]) -> list[str]:
    """Insert one evidence document and chunk per vector into the schema-v2 side table, returning chunk IDs."""
    import turso
...
class DedupEvidenceOfflineTest(IsolatedDBTest):
    def _db_path(self) -> str:
        return str(self.k1_payload["schema"]["db_path"])
    ...
    def test_threshold_must_be_within_cosine_range(self):
        for threshold in ("-0.01", "1.01"):
            with self.subTest(threshold=threshold):
                rc, payload = self.kz("O5", "--kind", "evidence", "--threshold", threshold, env=_UNCONFIGURED)
                self.assertEqual(rc, 2, payload)
                self.assertEqual(payload.get("code"), "DENIED_DEDUP_THRESHOLD", payload)

    def test_dimension_tie_uses_lower_dimension_and_reports_drops(self):
        db = self._db_path()
        _seed_evidence_vectors(db, [
            [1.0, 0.0], [0.99, 0.01],
            [1.0, 0.0, 0.0], [0.99, 0.01, 0.0],
        ])
        rc, payload = self.kz("O5", "--kind", "evidence", "--threshold", "0.9", env=_UNCONFIGURED)
        self.assertEqual(rc, 0, payload)
        self.assertEqual(payload["records"], 2, payload)
        self.assertEqual(payload["dropped_dimension_mismatch"], 2, payload)
        self.assertEqual(set(payload["top_clusters"][0]), {"ch_dedup_0", "ch_dedup_1"}, payload)
