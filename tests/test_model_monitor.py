...
sys.path.insert(0, str(Path(__file__).resolve().parents[1]))  # repo root -> import the module directly
...
class MonitorRenderTest(IsolatedDBTest):
    def test_human_render_draws_one_shot(self):
        ...
        proc = run(self.root, "B6", env=_DEAD_OLLAMA, timeout=30)
    ...
    def test_watch_falls_back_to_one_shot_without_tty(self):
        ...
        proc = run(self.root, "B6", "--watch", env=_DEAD_OLLAMA, timeout=30)
        self.assertEqual(proc.returncode, 0, proc.stderr)
        self.assertIn("backend monitor", proc.stdout)
