...
class PiiScanCliTest(IsolatedDBTest):
    ...
    def test_secret_in_summary_is_denied_before_persistence(self):
        rc, p = self.kz(
            "B5", "--prompt", "clean input", "--summary", _AWS_KEY, env=_UNCONFIGURED,
        )
        self.assertEqual(rc, 2, p)
...
class PiiAttachWiringTest(unittest.TestCase):
    ...
    def test_b2_attaches_pii_when_configured(self):
        ...
        self.assertIn("deadbeefcafe", tags)  # backend-provided span_hash is serialized
        self.assertNotIn("advisory prompt", tags)  # raw scanned input is never attached
