...
from kaizen_components.comfy_mcp import CANDIDATES, classify_tools, mcp_generate, score_candidate  # noqa: E402
...
class McpCliTest(IsolatedDBTest):
    ...
    def test_mcp_generation_redacts_summary_before_submit(self):
        args = types.SimpleNamespace(candidate="artokun", summary="AKIAIOSFODNN7EXAMPLE", template="default")
        with mock.patch("kaizen_components.comfy_mcp._endpoint", return_value="http://127.0.0.1:8188"), \
             mock.patch("kaizen_components.comfy_mcp.probe", return_value={}), \
             mock.patch("kaizen_components.comfy_mcp._mcp_submit") as submit:
            with self.assertRaises(KaizenDenied) as ctx:
                mcp_generate(args, {})
        self.assertEqual(ctx.exception.code, "DENIED_TRACE_REDACTION")
