...
class SentenceTransformersSelectionTest(IsolatedDBTest):
    """B1 doctor selection and clean denial when the optional sentence-transformers extra is absent."""
    def test_doctor_selects_sentence_transformers(self):
        rc, p = self.kz("B1", env=_ST_SELECT)
        self.assertEqual(rc, 0, p)
        self.assertTrue(p.get("embedding", {}).get("configured"), p)
...
class SemanticChunkerTest(IsolatedDBTest):
    """E3 neural and semantic chunker backend-availability gates."""
    def _ingest(self, env: dict) -> str:
        """Write the shared document, ingest it under the supplied environment, and return its document ID."""
