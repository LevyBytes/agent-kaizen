...
# reranker / PII / Qwen judge -- the plan's model implementations) with the shared Kaizen venv's Python, not
...
#   ./tests/run-backend-tests.sh --no-live   |   ./tests/run-backend-tests.sh --no-gui --no-hold   |   ./tests/run-backend-tests.sh -k judge
#
