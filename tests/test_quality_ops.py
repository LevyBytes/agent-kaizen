...
class QualityOpsTest(IsolatedDBTest):
    ...
    """Quality, evaluation, evidence, and proof operation contracts over an isolated DB."""
...
class JsonFileAndQ7KindTest(IsolatedDBTest):
    """Q2 --evidence-file JSON fallback plus Q7 --kind routing for eval runs and cases."""

    def test_q2_evidence_file_fallback_round_trips(self):
        evidence_path = self.root / "evidence.json"
        evidence_path.write_text('["tests"]', encoding="utf-8")
        rc, p = self.kz(
        ...
        self.assertEqual(rec["record"]["evidence_locations_json"], '["tests"]', rec)
